...
class UserInputValidator:
    ...
    def create_array_from_user_input(user_input: str):
        """
        Splits each character, if it is an integer, in the user input string
    ...
    def validate_user_input_is_comma_separated(user_input: str, start: int,
        ...
        Otherwise, creates a regex pattern that will accept only integers
        from 1 until the array length provided. If the user input contains
        more than one integer from 1 until the array length provided the
        pattern expects the integers to be separated by comma with optional
        space inbetween. After that it matches the user input against the
        regex pattern and returns a boolean value depending on the matching
        evaluation.
        """
        if len(user_input) == 0:
            return False
    ...
    def validate_user_input_is_a_number(user_input: str):
        """
        If the user is an empty string or whitespace, returns false. Returns
        a boolean value based on the evaluation if the user input is a digit.
        """
        if len(user_input) == 0:
            return False
        elif user_input == "0":
    ...
    def validate_user_input_is_a_decimal(user_input: str):
        """
        If the user is an empty string or whitespace, returns false.
        Replaces the "." in the user input string with an empty string and
        checks if the result of that is numeric. If it is numeric it returns
    ...
    def validate_input_before_parsing(cafeteria_items: list[CafeteriaItem],
        ...
        """
        Replaces the displayed text in the console depending on if the user
        is removing or adding an item then creates a while loop which
        validates if the provided input contains integers and if more than
        one they are separated by comma. If the validation fails the user
        will be prompted with a message stating the input is invalid.
    ...
    def validate_input_for_items(item: CafeteriaItem,
        ...
        """
        Replaces the displayed text in the console depending on if the user
        is removing or adding an item then creates a while loop that prompts
        the user how much of the selected item they wish to order or remove
        and validates if the provided input contains integers and if more
        than one they are separated by comma. If the validation fails the
        user will be prompted with a message stating the input is invalid.
        Otherwise, the loop will break and return the user input.
        """
        info_text = "order"
        if is_removing:
